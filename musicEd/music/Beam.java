...
public class Beam extends Mass{
    ...
    private void drawBeamGroup(Graphics g) {
        ...
        int nPrev = 0, nCur = firstStem.nFlag, nNext = stems.get(1).nFlag; // flag counts on previous, current and next
        int px; int cx = firstStem.X(); // the x location of the previous stem and current stem
        ...
        for(int cur = 1; cur < stems.size(); cur++) {
            ...
            if(nCur > nPrev && nCur > nNext) { // need beamlet on current stem
                if(nPrev < nNext) { // beamlets lean torwards signed beams
                    bx = cx + 3 * H;
